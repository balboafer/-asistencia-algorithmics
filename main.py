...
def normalize_mx_whatsapp(number: str) -> str:
    # Mexican mobile numbers need +521 prefix for WhatsApp
    if number.startswith('+52') and not number.startswith('+521') and len(number) == 13:
        return '+521' + number[3:]
    return number

def send_whatsapp(to_number: str, message: str):
    to_number = normalize_mx_whatsapp(to_number)
    try:
